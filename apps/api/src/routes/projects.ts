...
export default async function projectsRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await pool.query(
      'SELECT id, name, description, context, created_at, updated_at FROM projects WHERE user_id = $1 ORDER BY created_at DESC',
      [userId]
    );
    return result.rows;
  });
  ...
  fastify.post('/', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await pool.query(
      `INSERT INTO projects (user_id, name, description, context)
       VALUES ($1, $2, $3, $4)
       RETURNING id, name, user_id, description, context, created_at, updated_at`,
      [userId, name, description || '', context || {}]
    );
  ...
  fastify.put('/:id', async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const result = await pool.query(
      `UPDATE projects
       ...
       RETURNING *`,
      [name, description, context, id, userId]
    );
    if (result.rows.length === 0) {
      return reply.status(404).send({ error: 'Project not found' });
    }
